"""
ROI Calculator für KI-Implementierung - Verbesserte Version
Berechnungslogik mit realistischeren Parametern
"""
...
class CurrentSituation:
    ...
    affected_employees_percentage: float = 50.0  # % der Mitarbeiter, die von KI betroffen sind
...
class AIUseCase:
    ...
    # Verbesserungspotenziale (realistischere Werte)
...
class ROICalculator:
    ...
    def _initialize_use_cases(self) -> List[AIUseCase]:
        """Initialisiert alle verfügbaren KI-Use Cases mit realistischeren Werten"""
        return [
            AIUseCase(
                id="doc_processing",
                ...
                setup_cost=10000,
                monthly_cost=350,
                time_savings_percentage=40,  # Reduziert von 80%
                error_reduction_percentage=60,  # Reduziert von 90%
                industry_fit={
            ...
            AIUseCase(
                ...
                setup_cost=17500,
                monthly_cost=550,
                time_savings_percentage=30,  # Reduziert von 65%
                productivity_increase=20,  # Reduziert von 40%
                industry_fit={
            ...
            AIUseCase(
                ...
                setup_cost=6500,
                monthly_cost=275,
                response_time_improvement=50,  # Reduziert von 85%
                customer_satisfaction_increase=8,  # Reduziert von 15%
                time_savings_percentage=35,  # Reduziert von 70%
                industry_fit={
                    Industry.RETAIL: 0.9,
                    Industry.SERVICES: 0.9,
                    Industry.FINANCE: 0.8,
            ...
            AIUseCase(
                ...
                setup_cost=21000,
                monthly_cost=650,
                productivity_increase=15,  # Reduziert von 25%
                conversion_rate_increase=3,  # Reduziert von 8%
                industry_fit={
                    Industry.RETAIL: 0.9,
            ...
            AIUseCase(
                ...
                setup_cost=14000,
                monthly_cost=500,
                conversion_rate_increase=8,  # Reduziert von 22%
                customer_satisfaction_increase=5,  # Reduziert von 12%
                industry_fit={
                    Industry.RETAIL: 1.0,
            ...
            AIUseCase(
                ...
                setup_cost=10500,
                monthly_cost=425,
                time_savings_percentage=50,  # Reduziert von 90%
                productivity_increase=15,  # Reduziert von 30%
                industry_fit={
                    Industry.FINANCE: 0.9,
                    Industry.MANUFACTURING: 0.8,
                    Industry.SERVICES: 0.8,
    ...
    def calculate_pain_score(self, situation: CurrentSituation) -> Dict[str, float]:
        ...
        efficiency_pain = (
            ...
            (min(situation.processing_time_per_task / 60, 2) / 2) * 0.3 +  # Max 2h normalisiert
        ...
        # Kundenservice-Pain
        customer_pain = (
            (min(situation.avg_response_time / 48, 1)) * 0.5 +  # Max 48h normalisiert
        ...
        # Skalierungs-Pain
        scaling_pain = (
            (min(situation.support_tickets_per_month / 2000, 1)) * 0.6 +  # Max 2000 normalisiert
            (min(situation.avg_response_time / 24, 1)) * 0.4
        )
    ...
    def calculate_roi(self, company: CompanyProfile, situation: CurrentSituation, 
        ...
        """Berechnet ROI für einen spezifischen Use Case mit realistischeren Annahmen"""
        ...
        affected_employees = int(company.employee_count * (situation.affected_employees_percentage / 100))
        
        # Kosteneinsparungen berechnen (nur für betroffene Mitarbeiter)
        annual_labor_cost_affected = (affected_employees * situation.weekly_hours_per_employee * 
                                    52 * company.avg_hourly_wage)
        
        # Zeitersparnis (nur für manuelle Tätigkeiten der betroffenen Mitarbeiter)
        time_savings_factor = (use_case.time_savings_percentage / 100) * effectiveness
        manual_work_factor = situation.manual_work_percentage / 100
        annual_time_savings = annual_labor_cost_affected * time_savings_factor * manual_work_factor
        
        # Fehlerreduzierung (nur für betroffene Prozesse)
        error_reduction_factor = (use_case.error_reduction_percentage / 100) * effectiveness
        affected_tasks_per_year = situation.tasks_per_day * 365 * (affected_employees / company.employee_count)
        annual_error_savings = (affected_tasks_per_year * (situation.error_rate / 100) * 
                              error_reduction_factor * situation.cost_per_error)
        
        # Produktivitätssteigerung (konservativer Ansatz)
        productivity_factor = (use_case.productivity_increase / 100) * effectiveness * 0.5  # 50% Dämpfung
        annual_productivity_gain = annual_labor_cost_affected * productivity_factor
        
        # Umsatzsteigerungen (sehr konservativ)
        # Kundenservice-Verbesserung
        customer_improvement_factor = (use_case.customer_satisfaction_increase / 100) * effectiveness
        retention_improvement = customer_improvement_factor * 0.2  # Nur 20% der Zufriedenheit = Retention
        annual_retention_gain = (company.monthly_revenue * 12 * retention_improvement * 0.1)  # Nur 10% des Umsatzes betroffen
        
        # Conversion-Rate-Verbesserung
        conversion_improvement_factor = (use_case.conversion_rate_increase / 100) * effectiveness
        annual_conversion_gain = company.monthly_revenue * 12 * conversion_improvement_factor * 0.05  # Nur 5% des Umsatzes betroffen
        ...
        # ROI-Kennzahlen
        roi_percentage = ((total_annual_benefit - total_investment) / total_investment) * 100 if total_investment > 0 else 0
...
def create_sample_data():
    ...
    situation = CurrentSituation(
        ...
        affected_employees_percentage=60,  # 60% der Mitarbeiter betroffen
...
if __name__ == "__main__":
    # Test der verbesserten Berechnungslogik
    calculator = ROICalculator()
    company, situation = create_sample_data()
    
    print("=== ROI Calculator Test (Verbesserte Version) ===")
    ...
    # ROI Berechnung für Top Use Case in allen Szenarien
    top_use_case = recommendations[0]['use_case']
    print(f"\nROI Analyse für '{top_use_case.name}':")
    
    for scenario in ["conservative", "realistic", "optimistic"]:
        roi_result = calculator.calculate_roi(company, situation, top_use_case, scenario)
        ...
        print(f"    Betroffene Mitarbeiter: {roi_result['affected_employees']}")
